...
export const generateDivinationPrompt = (divinationResult) => {
  ...
  const { question, mainHexagram, changedHexagram, changingLines } = divinationResult;
  ...
  if (hasChangingLines) {
    prompt += `3. 變爻分析：解讀變爻的特殊意涵，及其對本卦的影響\n`;
    prompt += `4. 變卦啟示：分析變卦所反映的未來趨勢\n`;
  }
  
  prompt += `5. 建議與總結：給予實際可行的建議\n\n`;
